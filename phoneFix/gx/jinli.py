        ...
        for aa in sel1.css(".xl_phone a"):
            ...
            for bb in sel2.css(".peijian_list li"):
                ...
                for cc in sel3.xpath("//table[@class='MsoNormalTable']/tbody/tr[position()>2 and position()<last()]"):
                    price = str(cc.css("td:nth-child(2) p span span::text").extract_first()).strip() if cc.css(
                        "td:nth-child(2) p span span::text").extract_first() else str(cc.css("td:nth-child(2) p span::text").extract_first())[
                                                                                  1:].strip()
