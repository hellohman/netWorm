...
class HhNetworm:
    ...
    def getRes(self, url, method='g', result='c-s', data=None, verify=True, timeout=None, proxies=None, headers=None):
        ...
        try:
            if method == 'g':
                res = self.sess.get(url, params=data, **kwargs)
            elif method == 'p':
                res = self.sess.post(url, data=data, **kwargs)
    ...
    def dl_result(result, res):
        if result == 'j':
            ...
            return res.headers  # 请求头
        elif result == 'c':
            return res.content  # 二进制响应内容
        elif result == 'c-d':
            try:
            ...
            try:
                return Selector(res.content.decode('utf-8'))
            except:
                return Selector(res.content.decode('gb18030'))

    @staticmethod  # 辅助_错误衔接
    def dl_wrong_res(result):
        if result in ('sel', 'text-sel'):
            return Selector("")
        elif result in ('json', 'headers'):
            return {}
        elif result in ('text', 'content'):
            return ""
